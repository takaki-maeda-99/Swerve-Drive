...
// エンコーダー仕様と計算定数
const float ENCODER_RESOLUTION = 2048.0f;    // パルス/回転 (PPR)
...
void handleSerialCommand() {
  ...
  while (Serial.available()) {
      ...
      // 1. ヘッダーの同期 (0xAA)
      if (rx_idx == 0 && c != 0xAA) continue;
      // 2. ヘッダーの同期 (0x55)
      if (rx_idx == 1 && c != 0x55) { rx_idx = 0; continue; }
      rx_buf[rx_idx++] = c;
      // 3. パケットが全て揃ったら検証
      if (rx_idx >= sizeof(RobotCommandPacket)) {
          RobotCommandPacket* cmd = (RobotCommandPacket*)rx_buf;
          // チェックサム計算
          uint8_t sum = 0;
          for (size_t i = 0; i < sizeof(RobotCommandPacket) - 1; i++) {
              sum += rx_buf[i];
          }
          if (sum == cmd->checksum) {
              // 指示を機体速度に反映！
...
void loop() {
  ...
  if (Serial.availableForWrite() > 64) {   // 送れるときだけ
      ...
      //Serial.printf("Yaw: %.3f m/s, Pitch: %.3f m/s, Roll: %.3f rad/s\n", packet.yaw, packet.pitch, packet.roll);
      // 2. チェックサムの計算
          uint8_t sum = 0;
